...
const Hm17f = () => {
  ...
  const player = {
    geometry: new Capsule(
      new THREE.Vector3(-0.5, 0.15, -0.18),
      new THREE.Vector3(-0.5, 0.2, -0.18),
      0.05
    ),
  ...
  function loadCollision() {
    ...
    collision_loader.setDRACOLoader(dracoLoader);
  ...
  function resetPlayer() {
    player.geometry.start.set(-0.5, 0.05, -0.18);
    player.geometry.end.set(-0.5, 0.15, -0.18);
    player.geometry.radius = 0.02;
    camera.position.copy(player.geometry.end);
    camera.rotation.set(0, 0, 0);
  }

  function animateCamera() {
    const tween = new TWEEN.Tween(player.velocity).to({ y: -3 }, 2000).start();
  }
